...
new_route = """  app.post('/api/admin-outbox/send-bulk', async (req, res) => {
    try {
      ...
      if (isBroadcastMode) {
        for (const user of targetUsers) {
           ...
           else if (activeRole === 'cadre') { notifTitle = 'تبليغ الكادر التدريسي'; recRole = 'teacher'; }
        ...
        insertTickets.push({
          ...
          schoolId: schoolId || null,
          userId: targetId,
          role: activeRole === 'cadre' ? 'teacher' : activeRole,
      ...
      realtimeServerInstance?.broadcastToSchool(schoolId || '', 'notifications_updated', {});
      realtimeServerInstance?.broadcastToSchool(schoolId || '', 'support_tickets_updated', {});
      realtimeServerInstance?.broadcastToSchool(schoolId || '', 'admin_outbox_updated', {});
...
content = content.replace("  app.post('/api/admin-outbox', async (req, res) => {", new_route)

with open(filepath, 'w') as f:
    f.write(content)
print("Server send-bulk added")
