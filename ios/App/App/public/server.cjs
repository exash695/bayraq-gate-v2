...
function normalizeSectionLetter(letter) {
...
function extractSectionLetter(raw) {
  if (!raw || typeof raw !== "string") return null;
  const s = raw.replace(/[\u064B-\u065F\u0640]/g, "").trim();
...
function extractGradeBase(raw) {
  if (!raw || typeof raw !== "string") return "";
  let s = raw.replace(/[\u064B-\u065F\u0640]/g, "").replace(/\(([\u0621-\u064Aa-zA-Z0-9]+)\)/g, "").replace(/شعبة\s*[\u0621-\u064Aa-zA-Z0-9]+/g, "").replace(/[\s\-_–—]+[\u0621-\u064Aa-zA-Z0-9]$/, "").replace(/\s+/g, " ").trim();
  return s;
}
function matchesBroadcastAudience(broadcast, student) {
  ...
  const studentLetter = extractSectionLetter(studentSectionStr) || extractSectionLetter(studentClassStr) || extractSectionLetter(studentGradeStr);
  const studentGradeBase = extractGradeBase(studentGradeStr) || extractGradeBase(studentClassStr) || studentGradeStr;
  ...
  if (isSpecificSectionBroadcast) {
    const targetLetters = specificTargetSections.map((sec) => extractSectionLetter(sec)).filter(Boolean);
    ...
    const matchesAnySection = specificTargetSections.some((sec) => {
      ...
      const stripAl = (str) => str.replace(/\bال/g, "").replace(/\s+/g, "");
...
__export(schema_exports, {
  ...
  system_pose_history: () => system_pose_history,
  system_poses: () => system_poses,
...
var system_poses = (0, import_pg_core.pgTable)("system_poses", {
  id: (0, import_pg_core.varchar)("id", { length: 128 }).primaryKey(),
  publicUrl: (0, import_pg_core.text)("public_url").notNull(),
  category: (0, import_pg_core.varchar)("category", { length: 50 }).default("pose"),
  aliases: (0, import_pg_core.jsonb)("aliases").default([]),
  updatedBy: (0, import_pg_core.varchar)("updated_by", { length: 255 }),
  createdAt: (0, import_pg_core.timestamp)("created_at").defaultNow(),
  updatedAt: (0, import_pg_core.timestamp)("updated_at").defaultNow()
});
var system_pose_history = (0, import_pg_core.pgTable)("system_pose_history", {
  id: (0, import_pg_core.varchar)("id", { length: 128 }).primaryKey(),
  assetId: (0, import_pg_core.varchar)("asset_id", { length: 128 }).notNull(),
  fileName: (0, import_pg_core.text)("file_name"),
  downloadUrl: (0, import_pg_core.text)("download_url").notNull(),
  assetType: (0, import_pg_core.varchar)("asset_type", { length: 50 }),
  fileSize: (0, import_pg_core.integer)("file_size").default(0),
  uploadedBy: (0, import_pg_core.varchar)("uploaded_by", { length: 255 }),
  status: (0, import_pg_core.varchar)("status", { length: 50 }).default("active"),
  uploadedAt: (0, import_pg_core.timestamp)("uploaded_at").defaultNow()
});
var system_settings = (0, import_pg_core.pgTable)("system_settings", {
  key: (0, import_pg_core.varchar)("key", { length: 128 }).primaryKey(),
...
async function startServer() {
  ...
  let initSystemTablesPromise = null;
        ...
        try {
          ...
          if (existingRows.length === 0) {
            ...
            if (import_fs2.default.existsSync(POSES_FILE)) {
              ...
              const filePoses = JSON.parse(raw) || {};
  ...
  const fetchAllSystemPoses = async () => {
    try {
      ...
      try {
        const POSES_FILE = import_path2.default.join(process.cwd(), "data", "bairaq_poses.json");
        if (import_fs2.default.existsSync(POSES_FILE)) {
          return JSON.parse(import_fs2.default.readFileSync(POSES_FILE, "utf-8")) || {};
        }
      } catch (e) {
      }
      return {};
  ...
  app.post("/api/bairaq/poses", async (req, res) => {
    try {
      ...
      const userEmail = req.user?.email || "developer";
      ...
      try {
        ...
        import_fs2.default.writeFileSync(import_path2.default.join(DATA_DIR, "bairaq_poses.json"), JSON.stringify(currentPoses, null, 2), "utf-8");
      } catch (e) {
      }
  ...
  app.get("/api/bairaq/history/:assetId", async (req, res) => {
    try {
      ...
      const matchKeys = Array.from(/* @__PURE__ */ new Set([assetId, ...aliases]));
      const rows = await withDbRetry(
        () => db.select().from(system_pose_history).where((0, import_drizzle_orm.inArray)(system_pose_history.assetId, matchKeys)).orderBy((0, import_drizzle_orm.desc)(system_pose_history.uploadedAt)).limit(100)
      );
      const records = [];
  ...
  app.post("/api/bairaq/reset", async (req, res) => {
    try {
      ...
      await withDbRetry(
        () => db.delete(system_poses).where((0, import_drizzle_orm.inArray)(system_poses.id, keysToReset))
      );
      const currentPoses = await fetchAllSystemPoses();
      if (realtimeServerInstance) {
        realtimeServerInstance.broadcastManual("bairaq_poses", assetId, "UPDATE", currentPoses);
  ...
  app.get(["/api/system_config/:id", "/api/system_config"], async (req, res) => {
    try {
      ...
      const id = req.params.id || req.query.id || "remote_control";
      const rows = await withDbRetry(
        () => db.select().from(system_settings).where((0, import_drizzle_orm.eq)(system_settings.key, id)).limit(1)
      );
      if (rows.length > 0) {
    ...
    try {
      ...
      try {
        ...
        try {
          const POSES_FILE = import_path2.default.join(process.cwd(), "data", "bairaq_poses.json");
          if (import_fs2.default.existsSync(POSES_FILE)) {
            poses = JSON.parse(import_fs2.default.readFileSync(POSES_FILE, "utf-8")) || {};
          }
        } catch (fErr) {
        }
      }
...
\u064A\u062C\u0628 \u0623\u0644\u0627 \u064A\u0636\u064A\u0639 \u0623\u064A \u062D\u0631\u0641 \u0639\u0644\u0645\u064A \u0645\u0646 \u0647\u0630\u0627 \u0627\u0644\u0646\u0635\u060C \u0627\u0646\u0642\u0644\u0647 \u0643\u0645\u0627 \u0647\u0648 \u062A\u0645\u0627\u0645\u0627\u064B.` : ""}
  ...
  app.get("/api/schools/:id", async (req, res) => {
    try {
      const { id } = req.params;
      const schoolList = await db.select().from(schools).where((0, import_drizzle_orm.eq)(schools.id, id)).limit(1);
      if (schoolList.length === 0) {
        return res.status(404).json({ success: false, message: "School not found" });
      }
  ...
  const handleUpdateSchool = async (req, res) => {
    try {
      const { id } = req.params;
      const body = req.body || {};
      const mapped = {};
      if (body.name !== void 0) mapped.name = body.name.trim();
      if (body.governorate !== void 0) mapped.governorate = body.governorate;
      if (body.location !== void 0) mapped.location = body.location;
      if (body.coverUrl !== void 0 || body.schoolBairaqImageUrl !== void 0) {
        mapped.coverUrl = body.coverUrl || body.schoolBairaqImageUrl;
      }
      if (body.logoUrl !== void 0 || body.schoolLogoUrl !== void 0) {
        mapped.logoUrl = body.logoUrl || body.schoolLogoUrl;
      }
      if (body.status !== void 0) mapped.status = body.status;
      if (body.type !== void 0) mapped.type = body.type;
      if (body.disabledModules !== void 0) mapped.disabledModules = body.disabledModules;
      const existing = await db.select().from(schools).where((0, import_drizzle_orm.eq)(schools.id, id)).limit(1);
      let resultSchool;
      if (existing.length > 0) {
        if (Object.keys(mapped).length > 0) {
          const updated = await db.update(schools).set(mapped).where((0, import_drizzle_orm.eq)(schools.id, id)).returning();
  ...
  app.delete("/api/schools/:id", requireDeveloper, async (req, res) => {
    try {
      const { id } = req.params;
      await db.delete(schools).where((0, import_drizzle_orm.eq)(schools.id, id));
  ...
  app.get("/api/broadcasts", async (req, res) => {
    try {
      ...
      const enhancedResults = results.map((r) => {
        ...
        const detectedSections = rawGrades.filter((g) => typeof g === "string" && /[\s\-_–—]+[\u0621-\u064Aa-zA-Z0-9]$|\([\u0621-\u064Aa-zA-Z0-9]+\)|شعبة\s*[\u0621-\u064Aa-zA-Z0-9]+/.test(g));
        return {
          ...r,
          targetSection: r.targetSection || (detectedSections.length === 1 ? detectedSections[0] : null),
          targetSections: r.targetSections || (detectedSections.length > 0 ? detectedSections : [])
        };
      });
      ...
      if (grade && typeof grade === "string" && grade.trim() !== "") {
        ...
        filteredResults = enhancedResults.filter((b) => matchesTargetGrades2(grade, b.targetGrades));
      }
      ...
      if (messageQuery && messageQuery.trim()) {
        await db.delete(school_announcements).where(
          (0, import_drizzle_orm.or)(
            (0, import_drizzle_orm.eq)(school_announcements.id, id),
            (0, import_drizzle_orm.eq)(school_announcements.message, messageQuery.trim())
          )
        );
      } else {
  ...
  app.post("/api/auth/login-code", async (req, res) => {
    try {
          ...
          for (const aList of allLists) {
            ...
            if (foundStu) {
              ...
              studentList = [{
                ...
                grade: resolvedGrade,
                section: aList.name || foundStu.section || "",
                class: aList.name || foundStu.class || "",
                className: aList.name || foundStu.className || "",
      ...
      if (studentList.length > 0) {
        ...
        let studentSection = stu.section || "";
        let studentClassName = stu.className || stu.class || "";
        try {
          const effectiveSchool = stu.schoolId || targetSchoolId || "school1";
          const schoolLists = await db.select({ id: academic_lists.id, name: academic_lists.name, students: academic_lists.students }).from(academic_lists).where((0, import_drizzle_orm.eq)(academic_lists.schoolId, effectiveSchool));
          for (const aList of schoolLists) {
            const listStudents = Array.isArray(aList.students) ? aList.students : [];
            const foundInList = listStudents.some((s) => {
              const scode = String(s.student || s.code || "").trim().toUpperCase();
              const sid = String(s.id || "").trim().toUpperCase();
        ...
        return res.json({
          ...
          user: {
            ...
            section: studentSection || stu.section || "",
            class: studentClassName || studentSection || stu.class || "",
            className: studentClassName || studentSection || stu.className || "",
  ...
  app.post("/api/auth/login", async (req, res) => {
    try {
      ...
      try {
        ...
        userList = await db.select().from(users).where((0, import_drizzle_orm.or)(
  ...
  app.post("/api/auth/delete-account", async (req, res) => {
    try {
      const userId = req.body?.userId || req.user?.uid || req.user?.id;
      const userEmail = (req.body?.email || req.user?.email || "").trim().toLowerCase();
      const userCode = req.body?.code || req.user?.studentCode;
      ...
      if (userId) {
        ...
        await db.delete(students).where((0, import_drizzle_orm.eq)(students.id, userId));
        await db.delete(teachers).where((0, import_drizzle_orm.eq)(teachers.id, userId));
        await db.delete(activation_codes).where((0, import_drizzle_orm.eq)(activation_codes.id, userId));
        await db.delete(attendance_logs).where((0, import_drizzle_orm.eq)(attendance_logs.studentId, userId));
        await db.delete(behavior_logs).where((0, import_drizzle_orm.eq)(behavior_logs.studentId, userId));
        await db.delete(student_transactions).where((0, import_drizzle_orm.eq)(student_transactions.studentId, userId));
        await db.delete(user_device_tokens).where((0, import_drizzle_orm.eq)(user_device_tokens.userId, userId));
        await db.delete(firestore_docs).where((0, import_drizzle_orm.and)((0, import_drizzle_orm.eq)(firestore_docs.collection, "users"), (0, import_drizzle_orm.eq)(firestore_docs.docId, userId)));
      }
      if (userEmail) {
        await db.delete(users).where((0, import_drizzle_orm.eq)(users.email, userEmail));
      }
      if (userCode) {
      ...
      if (schoolId && userCode) {
        ...
        await db.delete(students).where((0, import_drizzle_orm.eq)(students.id, compId));
        await db.delete(attendance_logs).where((0, import_drizzle_orm.eq)(attendance_logs.studentId, compId));
        await db.delete(behavior_logs).where((0, import_drizzle_orm.eq)(behavior_logs.studentId, compId));
        await db.delete(student_transactions).where((0, import_drizzle_orm.eq)(student_transactions.studentId, compId));
      }
      if (userId) {
...
(\u0635\u0627\u0644\u062D \u0644\u0645\u062F\u0629 15 \u062F\u0642\u064A\u0642\u0629. \u0644\u0627 \u062A\u0634\u0627\u0631\u0643 \u0647\u0630\u0627 \u0627\u0644\u0631\u0645\u0632 \u0645\u0639 \u0623\u064A \u0634\u062E\u0635).`
  ...
  app.post("/api/auth/register-phone/request", async (req, res) => {
    try {
      ...
      const existingUser = await db.select().from(users).where((0, import_drizzle_orm.or)(
        (0, import_drizzle_orm.eq)(users.phone, cleanPhone),
        (0, import_drizzle_orm.eq)(users.email, placeholderEmail)
      )).limit(1);
      const otp = Math.floor(1e5 + Math.random() * 9e5).toString();
      ...
      if (existingUser.length > 0) {
        ...
        await db.update(users).set({
          ...
          whatsappOtp: otp,
          whatsappOtpExpires: otpExpires
        }).where((0, import_drizzle_orm.eq)(users.id, tempId));
      } else {
        ...
        try {
          ...
          await fetch(endpoint, {
            ...
            body: JSON.stringify({
              token: waToken,
              to: recipientPhone,
              body: `\u0645\u0631\u062D\u0628\u0627\u064B \u0628\u0643 \u0641\u064A \u0628\u0648\u0627\u0628\u0629 \u0628\u064A\u0631\u0642 \u{1F6E1}\uFE0F
...
(\u0635\u0627\u0644\u062D \u0644\u0645\u062F\u0629 10 \u062F\u0642\u0627\u0626\u0642)`
  ...
  app.post("/api/auth/register-phone/verify", async (req, res) => {
    try {
      ...
      const userList = await db.select().from(users).where((0, import_drizzle_orm.eq)(users.id, tempId)).limit(1);
      if (userList.length === 0) {
      ...
      if (user.whatsappOtpExpires && new Date(user.whatsappOtpExpires) < /* @__PURE__ */ new Date()) {
